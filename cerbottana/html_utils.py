...
async def image_url_to_html(url: str) -> Doc:
    """Generates an <img> tag from an image url."""
    image = await probe(url)
    doc = get_doc()
    doc.stag("img", src=url, width=image.width, height=image.height)
    return doc
...
def to_obfuscated_html(text: str) -> Doc:
    ...
    for ch in text:
        ...
        doc.line("span", randstr, style="position: absolute; top: -999vh")
    return doc
...
class HTMLPageCommand(BaseHTMLCommand):
    ...
    def __init__(
        ...
        *,
        title: str,
        fields: list[tuple[str, str | Callable[[Row, Doc], None]]],
        actions_header: str = "",
        actions: list[
            tuple[
                str,  # cmd
    ...
    def load_page(self, page: int) -> None:
        ...
        with db.get_session() as session:
            ...
            with tag("div", klass="pad"):
                line("h2", self._title)
                if not rs:
                    text("No results found")
                    return

                with tag("div", klass="ladder"), tag("table"):
                    with tag("tr"):
                        for field_header, _ in self._fields:
                            line("th", field_header)
                        line("th", self._actions_header)
                            ...
                            for _, field in self._fields:
                                with tag("td"):
                                    if isinstance(field, str):
                                        text(str(getattr(row[0], field) or ""))
                                    else:
                                        field(row, doc)

                            with tag("td", style=self._get_css("one_pixel_width")):
                ...
                for p in range(last_page):
                    with tag("button", klass="option"):
                        if page == p + 1:
                            doc.add_class("sel")
                            doc.attr(disabled="")
                        else:
                            doc.attr(name="send", value=f"{page_cmd}{p + 1}")
                        text(str(p + 1))
    ...
    def _add_action_buttons(self, row: Row) -> None:
        ...
        for cmd, params, disabled, btn_icon, btn_text in self._actions:
            ...
            with tag("button", klass="button"):
                if not isinstance(disabled, bool):
                    disabled = disabled(row)
                if disabled:
                    doc.add_class("disabled")
                    doc.attr(style=self._get_css("btn_disabled"))
                else:
                    doc.attr(name="send", value=cmd)
                if btn_icon:
                    line("i", "", klass=f"fa fa-{btn_icon}")
                text(f" {btn_text}")
            text(" ")
